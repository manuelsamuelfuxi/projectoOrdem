...
<div style="max-width: 680px;">
    ...
    <div style="background:white; border:0.5px solid #e2e8f0; border-radius:12px; margin-bottom:20px;">
            ...
            <form method="POST" action="{{ route('admin.perfil.atualizar') }}">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="name" value="{{ old('name', $utilizador->name) }}"
                        style="width:100%; padding:9px 12px; border:0.5px solid {{ $errors->has('name') ? '#fca5a5' : '#e2e8f0' }}; border-radius:8px; font-size:13.5px; outline:none;">
                    @error('name')
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="email" name="email" value="{{ old('email', $utilizador->email) }}"
                        style="width:100%; padding:9px 12px; border:0.5px solid {{ $errors->has('email') ? '#fca5a5' : '#e2e8f0' }}; border-radius:8px; font-size:13.5px; outline:none;">
                    @error('email')
                        <div style="font-size:12px; color:#ef4444; margin-top:4px;">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <div style="background:white; border:0.5px solid #e2e8f0; border-radius:12px;">
        ...
        <div style="padding:24px;">
            <form method="POST" action="{{ route('admin.perfil.senha') }}">
                @csrf
                @method('PUT')

                <div style="margin-bottom:16px;">
                    <label style="display:block; font-size:13px; font-weight:500; color:#374151; margin-bottom:6px;">Senha actual</label>
                    <input type="password" name="senha_actual"
                        style="width:100%; padding:9px 12px; border:0.5px solid {{ $errors->has('senha_actual') ? '#fca5a5' : '#e2e8f0' }}; border-radius:8px; font-size:13.5px; outline:none;"
                        placeholder="••••••••">
                    @error('senha_actual')
                        <div style="font-size:12px; color:#ef4444; margin-top:4px;">{{ $message }}</div>
                    @enderror
                ...
                <div style="margin-bottom:16px;">
                    <label style="display:block; font-size:13px; font-weight:500; color:#374151; margin-bottom:6px;">Nova senha</label>
                    <input type="password" name="nova_senha"
                        style="width:100%; padding:9px 12px; border:0.5px solid {{ $errors->has('nova_senha') ? '#fca5a5' : '#e2e8f0' }}; border-radius:8px; font-size:13.5px; outline:none;"
                        placeholder="••••••••">
                    @error('nova_senha')
                        <div style="font-size:12px; color:#ef4444; margin-top:4px;">{{ $message }}</div>
                    @enderror
                ...
                <div style="margin-bottom:24px;">
                    <label style="display:block; font-size:13px; font-weight:500; color:#374151; margin-bottom:6px;">Confirmar nova senha</label>
                    <input type="password" name="nova_senha_confirmation"
                        style="width:100%; padding:9px 12px; border:0.5px solid #e2e8f0; border-radius:8px; font-size:13.5px; outline:none;"
                        placeholder="••••••••">
                </div>

                <div style="display:flex; justify-content:flex-end;">
                    <button type="submit"
                        style="padding:9px 20px; background:#0f172a; color:white; border:none; border-radius:8px; font-size:13px; font-weight:500; cursor:pointer;">
